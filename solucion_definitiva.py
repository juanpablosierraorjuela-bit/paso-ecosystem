...
# --- 1. FUNCIÓN DE DISTANCIA (ESTRICTAMENTE PROTEGIDA) ---
def haversine_distance(lon1, lat1, lon2, lat2):
    try:
        if lon1 in [None, ''] or lat1 in [None, ''] or lon2 in [None, ''] or lat2 in [None, '']:
            return float('inf')
        
        # Forzar conversión a string, cambiar comas por puntos, y luego a float
        ...
        print(f"Error silencioso en haversine: {e}")
        return float('inf') 
...
def marketplace(request):
    ...
    try:
        ...
        try:
            ...
            print(f"⚠️ Error recuperable en Geolocalización: {e_geo}")
            pass
        ...
        for salon in salones_para_mostrar:
            ...
            salones_finales.append(salon)

        # IMPORTANTE: Renderiza home.html que es el buscador
        return render(request, 'home.html', {
            'salones': salones_finales,
            'user_located': user_located,
        })

    except Exception as e_critical:
        print(f"🔥 ERROR CRÍTICO EN MARKETPLACE: {e_critical}")
