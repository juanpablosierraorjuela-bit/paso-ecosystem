...
def check_availability(employee, start_time, duration_minutes):
    ...
    # Validar rangos de horario (Inicio y Fin de jornada)
    ...
    # Si la cita choca con el almuerzo
    if employee.lunch_start and employee.lunch_end:
        # Se cruza si inicia antes de que termine el almuerzo Y termina después de que empiece
    ...
    # Buscamos citas 'confirmed' que se solapen
...
def create_booking_service(salon, service, customer, form_data):
    ...
    employee = form_data.get('employee') # Puede ser None si eligió "Cualquiera"
    ...
    if employee:
        ...
        # El usuario eligió "Cualquiera" -> Buscamos el primero disponible
    ...
    with transaction.atomic():
        # Doble verificación final dentro de la transacción por seguridad (Race Conditions)
