...
# --- 1. LIMPIEZA DE REQUIREMENTS.TXT ---
def limpiar_requirements():
    print("🧹 1. Limpiando requirements.txt de librerías pesadas...")
    if os.path.exists(REQ_PATH):
        with open(REQ_PATH, "r", encoding="utf-8") as f:
            lines = f.readlines()
        
        # Librerías esenciales para Django en Render
        keep_libs = [
            "Django", "gunicorn", "psycopg2-binary", "dj-database-url", 
            "whitenoise", "pillow", "django-cors-headers"
        ]
        
        new_lines = []
        for line in lines:
            lib_name = line.split("==")[0].split(">")[0].strip()
            # Filtramos flet, pygame, pandas, numpy, etc.
            if lib_name.lower() in ["flet", "flet-desktop", "pygame", "pandas", "numpy", "matplotlib"]:
                continue
            new_lines.append(line)
            
        with open(REQ_PATH, "w", encoding="utf-8") as f:
            f.writelines(new_lines)
        print("   -> requirements.txt optimizado (se eliminaron flet, pygame, etc).")

# --- 2. REESCRIBIR FORMS.PY (Completo y limpio) ---
...
# --- 3. REESCRIBIR VIEWS.PY (Completo con CRUD de Servicios) ---
...
# --- 4. REESCRIBIR URLS.PY (Completo y enlazado) ---
...
if __name__ == "__main__":
    print("🚑 INICIANDO REPARACIÓN DE BUILD Y SERVICIOS 🚑")
    limpiar_requirements()
    reescribir_archivos()
    print("\n✅ Archivos saneados y optimizados. ¡Sube esto a GitHub ya!")