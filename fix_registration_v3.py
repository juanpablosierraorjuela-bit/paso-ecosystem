...
# -----------------------------------------------------------------------------
# 1. ARREGLAR FORMS.PY (Añadir Ciudad, Quitar Passwords de Meta)
# -----------------------------------------------------------------------------
forms_path = os.path.join("apps", "businesses", "forms.py")
print(f" Reparando formulario en {forms_path}...")
...
# -----------------------------------------------------------------------------
# 2. ARREGLAR VIEWS.PY (Mapeo de 'phone' a 'whatsapp_number' y Dashboard)
# -----------------------------------------------------------------------------
views_path = os.path.join("apps", "businesses", "views.py")
print(f" Corrigiendo lógica de guardado en {views_path}...")
...
# --- VISTAS AUTH (AQUÍ ESTÁ EL ARREGLO) ---
...
def register_owner(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # CORRECCIÓN: 'phone' del form se guarda en 'whatsapp_number' del modelo
            # CORRECCIÓN 2: 'city' ya viene en cleaned_data
...
# 3. SUBIR A GITHUB
# -----------------------------------------------------------------------------
print(" Subiendo solución definitiva del registro a GitHub...")
try:
    subprocess.run(["git", "add", "."], check=True)
    subprocess.run(["git", "commit", "-m", "Fix: Registro dueño funcionando (WhatsApp Map) + City Dropdown"], check=True)
    subprocess.run(["git", "push", "origin", "main"], check=True)
    print(" ¡LISTO! Prueba registrarte en 3 minutos.")
except Exception as e:
